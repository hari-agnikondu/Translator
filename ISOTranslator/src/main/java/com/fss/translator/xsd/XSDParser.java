...
/**
 * This class parses all or only some elements of a xsd. It returns XSDElements
 * and XSDAttributes.
 * 
...
public class XSDParser implements DOMErrorHandler {
	...
	private static XSDElement parseXSD(InputStream inputStream, List<String> elements, boolean allElements)
		...
		log.info("Main Element in XSD parser is {}", mainElement);
		return mainElement;
	}
	...
	public boolean handleError(DOMError error) {
		...
		if (severity == DOMError.SEVERITY_ERROR) {
			log.error("[xs-error]: ", error.getMessage());
		}

		if (severity == DOMError.SEVERITY_WARNING) {
			log.error("[xs-warning]: ", error.getMessage());
		}
		return true;
	}
